                ...
                () => {
                    ...
                    for (const a of links) {
                        ...
                        if (href && href.includes('/company/')) {
                            const cleanUrl = href.split('?')[0]; // 清除 ? 及後面的參數 (例如 ?jobsource)
                            
                            if (seen.has(cleanUrl)) {
                                continue;
                            }

                            // 尋找最近的公司區塊容器，用來判斷是否「已關閉」
                            const container = a.closest('.company-list') || a.closest('.row') || a.closest('article');
                            if (container) {
                                const buttons = container.querySelectorAll('button');
                                let isClosed = false;
                                for (const btn of buttons) {
                                    if (btn.textContent && btn.textContent.includes('已關閉')) {
                                        isClosed = true;
                                        break;
                                    }
                                }
