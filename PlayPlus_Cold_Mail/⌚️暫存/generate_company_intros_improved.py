...
# -*- coding: utf-8 -*-
"""
自動為 名單副本.csv 中的公司重新撰寫簡介內容（I欄），並合併寫入 temporary_104.json。
"""
...
def main():
    ...
    for idx, (company_name, original_desc) in enumerate(unique_companies.items(), 1):
        ...
        if cached_item and isinstance(cached_item, dict) and (cached_item.get("summary") or cached_item.get("new_description")):
        ...
        if summary_text:
            ...
            # 合併寫入暫存，不覆蓋既有欄位（例如電郵標題與內容）
            if company_name not in cache_data or not isinstance(cache_data[company_name], dict):
                cache_data[company_name] = {}
                
            cache_data[company_name]["company_name"] = company_name
            cache_data[company_name]["original_description"] = original_desc
            cache_data[company_name]["new_description"] = summary_text
            cache_data[company_name]["summary"] = summary_text
    ...
    # 一次性寫回 CSV 檔案，確保不新增額外欄位，保持 21 欄結構，完全不填寫或覆寫 Z 欄和 AA 欄
