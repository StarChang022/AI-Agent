...
此指令搭配 [[⚙️參數設定/writer_coldmail_python/writer_day60.py]] 使用，請完成 `# 任務` 中的任務。

# 任務

1. 熟讀 [[原則與攻略/冷郵件守則.md]] 的 `### Day 60：優雅退場` 原則。
2. 從 Google Sheets 讀取 [[名單副本]] 分頁的所有公司資料（公司品牌簡稱、email、聯絡人名稱、說明等欄位）。
3. 依照 `# 撰寫規定` 為每一間公司撰寫 Day 60 郵件標題（`day60_title`）與內容（`day60_content`）。
4. 將撰寫好的資料填入 [[⚙️參數設定/writer_coldmail_python/writer_day60.py]] 腳本的 `MAIL_DATA` 清單中。
...
2. 依照 `# 撰寫規定` 為每間公司個別撰寫 Day 60 標題與內容（宣告不再主動聯繫，利用失去感引發最後回應，優雅開放未來合作大門）。
3. 打開 [[⚙️參數設定/writer_coldmail_python/writer_day60.py]]，將撰寫好的資料填入 `MAIL_DATA` 清單：
   ```python
   MAIL_DATA = [
       {
           "公司品牌簡稱": "公司A",
...
4. 在終端機執行：
   ...
   ```
5. 腳本會自動將 `day60_title` 與 `day60_content` 寫入 Google Sheets 對應列。
...
- Day 60 為優雅退場，語氣需溫和尊重，宣告關閉追蹤案，並留下「未來歡迎聯絡」的開放大門。
