...
此指令搭配 [[⚙️參數設定/writer_coldmail_python/writer_day30.py]] 使用，請完成 `# 任務` 中的任務。

# 任務

1. 熟讀 [[原則與攻略/冷郵件守則.md]] 的 `### Day 30：處理潛在異議` 原則。
2. 從 Google Sheets 讀取 [[名單副本]] 分頁的所有公司資料（公司品牌簡稱、email、聯絡人名稱、說明等欄位）。
3. 依照 `# 撰寫規定` 為每一間公司撰寫 Day 30 郵件標題（`day30_title`）與內容（`day30_content`）。
4. 將撰寫好的資料填入 [[⚙️參數設定/writer_coldmail_python/writer_day30.py]] 腳本的 `MAIL_DATA` 清單中。
...
2. 依照 `# 撰寫規定` 為每間公司個別撰寫 Day 30 標題與內容（主動拆解常見顧慮：怕貴、怕花時間，強調模組化與分階段方案）。
3. 打開 [[⚙️參數設定/writer_coldmail_python/writer_day30.py]]，將撰寫好的資料填入 `MAIL_DATA` 清單：
   ```python
   MAIL_DATA = [
       {
           "公司品牌簡稱": "公司A",
...
4. 在終端機執行：
   ...
   ```
5. 腳本會自動將 `day30_title` 與 `day30_content` 寫入 Google Sheets 對應列。
...
- Day 30 為處理異議，需主動提及「模組化開發」、「分階段優化」等降低門檻的方案，並說明溝通成本低。
