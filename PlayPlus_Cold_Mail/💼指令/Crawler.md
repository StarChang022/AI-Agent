我現在需要為初期名單安排爬蟲作業，依序收集公司網址、公司說明與網域信箱。所有的資料狀態皆由各個 Python 腳本直接透過 API 與 Google Sheets 同步更新。
...
* 任務目標：執行對應腳本，自動讀取 Google Sheets `名單副本` 分頁，尋找公司網址並寫回表單。
* 驗收標準：終端機腳本執行完畢且無錯誤，即進入下一階段。

**階段 2：撰寫公司說明**

* 執行指令參考：@[💼指令/Crawler/3.104crawler撰寫公司說明.md]
* 任務目標：執行對應腳本，針對表單內的資料生成或爬取公司說明，並更新至 Google Sheets。
* 驗收標準：終端機腳本執行完畢且無錯誤，即進入下一階段。
