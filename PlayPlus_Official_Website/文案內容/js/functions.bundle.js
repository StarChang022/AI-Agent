...
(function (global, factory) {
	...
	var Core = function() {
		...
		viewport: function() {
			// 優先使用 window 屬性（不會觸發重排），避免讀取 DOM 幾何屬性
			var viewportWidth = window.innerWidth;
			var viewportHeight = window.innerHeight;

			// 只有在 window 屬性不可用時才讀取 DOM 屬性（會觸發重排）
			// 但這種情況極少發生，現代瀏覽器都支援 window.innerWidth/innerHeight
			if( !viewportWidth || !viewportHeight ) {
				viewportWidth = viewportWidth || vars.elRoot.clientWidth;
				viewportHeight = viewportHeight || vars.elRoot.clientHeight;
			}
			...
			// 批量讀取 body 高度（必須讀取 DOM 屬性，會觸發一次重排）
			// 在讀取所有幾何屬性後，再進行任何 DOM 寫入操作
			var bodyHeight = vars.elBody.clientHeight;

			// 更新內部變數
			vars.viewport = viewport;

			// 批量設置所有 CSS 變數，確保在同一個渲染週期內完成
			// 使用單一 DOM 操作來減少重排次數
			var rootEl = document.documentElement;
			rootEl.style.setProperty('--cnvs-viewport-width', viewport.width);
			rootEl.style.setProperty('--cnvs-viewport-height', viewport.height);
			rootEl.style.setProperty('--cnvs-body-height', bodyHeight);
		...
		StickFooterOnSmall: function() {
			return {
				...
				if( __core.getVars.elAppMenu ) {
					// 先批量讀取所有幾何屬性，避免強制重排
					var viewportHeight = __core.viewport().height;
					var appMenuRect = __core.getVars.elAppMenu.getBoundingClientRect();
					var appMenuHeight = __core.getVars.elAppMenu.offsetHeight;

					if((viewportHeight - (appMenuRect.top + appMenuRect.height)) === 0) {
						__core.getVars.elFooter.style.marginBottom = appMenuHeight+'px';
					}
				}
		...
		SliderParallax: function() {
			...
			var _parallax = function(_settings) {
				...
				fastdom.measure( function(){
					...
					var sliderHeight = _settings.sliderPx.el.offsetHeight;

					if( _settings.body.classList.contains('is-expanded-menu') && !_settings.isMobile ) {
						if( ( sliderHeight + _settings.sliderPx.offset + 50 ) > _settings.scrollPos.y ){
			...
			var _offset = function() {
				...
				if( elHeader ) {
					headerHeight = elHeader.offsetHeight || 0;
				}

				if( __core.getVars.elBody.classList.contains('side-header') || (elHeader && __core.getNext(elHeader, '.include-header').length > 0) ) {
					headerHeight = 0;
				}
			...
			var _elementFade = function(_settings) {
				...
				fastdom.measure( function(){
					// 先批量讀取所有幾何屬性，避免強制重排
					var sliderHeight = _settings.sliderPx.el.offsetHeight;
					var scrollPosY = _settings.scrollPos.y;
					var tHeaderOffset;
					...
					fastdom.mutate( function(){
						if( _settings.sliderPx.el.classList.contains('slider-parallax-visible') ) {
							_settings.sliderPx.el.querySelectorAll('.slider-arrow-left,.slider-arrow-right,.slider-caption,.slider-element-fade').forEach( function(el) {
								el.style.opacity = 1 - ( ( ( scrollPosY - tHeaderOffset ) * 1.85 ) / sliderHeight );
							});
						} else {
							_settings.sliderPx.el.querySelectorAll('.slider-arrow-left,.slider-arrow-right,.slider-caption,.slider-element-fade').forEach( function(el) {
								el.style.opacity = 1;
			...
			return {
				init: function(selector) {
					...
					__core.isFuncTrue( function() {
						...
						// 初始化時讀取一次 height，避免在 scroll 事件中頻繁讀取導致強制重排
						var sliderPxEl = __core.getVars.sliderParallax.el;
						_settings = {
							...
							height: sliderPxEl ? sliderPxEl.offsetHeight : 0,
							get classes() {
						...
						__core.getVars.resizers.sliderparallax = function() {
							// 在 resize 時更新 height，避免使用 getter 導致強制重排
							if( sliderPxEl ) {
								_settings.height = sliderPxEl.offsetHeight;
							}
							_parallax(_settings);
							_elementFade(_settings);
						};
