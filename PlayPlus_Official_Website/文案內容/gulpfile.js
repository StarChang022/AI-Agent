...
function copyOtherFiles() {
	...
	// 處理 public 資料夾內的檔案 (包含隱藏資料夾 .well-known)
	const publicFiles = gulp.src([
		'./public/**/*',
		'./public/.well-known/**/*'
	], { allowEmpty: true, dot: true, encoding: false, base: './public' })
		.pipe(gulp.dest('./dist'));

	// Gulp 4+ 支援非同步，可以使用 Promise 或串接
	return new Promise((resolve, reject) => {
		let count = 0;
		const done = () => { count++; if (count === 2) resolve(); };
		rootFiles.on('end', done).on('error', reject);
		publicFiles.on('end', done).on('error', reject);
	});
}
