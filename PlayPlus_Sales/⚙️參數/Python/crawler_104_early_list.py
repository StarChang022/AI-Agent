...
URL_LIST_FILE = os.path.join(BASE_DIR, '104_early_list.csv')          # 讀取網址清單
TEMP_FILE     = os.path.join(BASE_DIR, '104_early_list_temporary.csv') # 輸出暫存檔
        ...
        for i, url in enumerate(urls, 1):
            ...
            for attempt in range(1, 4):  # 最多重試 3 次
                try:
                    ...
                    # 截圖協助診斷（只在第一次失敗時）
                    if attempt == 1:
                        debug_path = os.path.join(BASE_DIR, f'debug_page_{i}.png')
                        try:
